...
const char *gafqX_token2str (LexState *ls, int token) {
  if (token < FIRST_RESERVED) {
    ...
    return (iscntrl(token)) ? gafqO_pushfstring(ls->L, "char(%d)", token) :
                              gafqO_pushfstring(ls->L, "%c", token);
  }
  else
...
void gafqX_syntaxerror (LexState *ls, const char *msg) {
  gafqX_lexerror(ls, msg, ls->t.token);
}
...
static void read_long_string (LexState *ls, SemInfo *seminfo, int sep) {
  ...
  for (;;) {
    switch (ls->current) {
      case EOZ:
        gafqX_lexerror(ls, (seminfo) ? "unfinished long string" :
      ...
      case '\r': {
        save(ls, '\n');
        inclinenumber(ls);
        if (!seminfo) gafqZ_resetbuffer(ls->buff);  /* avoid wasting space */
        break;
      }
      default: {
  ...
  if (seminfo)
    seminfo->ts = gafqX_newstring(ls, gafqZ_buffer(ls->buff) + (2 + sep),
                                     gafqZ_bufflen(ls->buff) - 2*(2 + sep));
}
...
static void read_string (LexState *ls, int del, SemInfo *seminfo) {
  ...
  seminfo->ts = gafqX_newstring(ls, gafqZ_buffer(ls->buff) + 1,
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '[': {
        ...
        else gafqX_lexerror(ls, "invalid long string delimiter", TK_STRING);
      }
      case '=': {
        next(ls);
        if (ls->current != '=') return '=';
      ...
      default: {
        if (isspace(ls->current)) {
          ...
          ts = gafqX_newstring(ls, gafqZ_buffer(ls->buff),
                                  gafqZ_bufflen(ls->buff));
